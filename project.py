...
from utils.util import set_target_gain, get_amplitude
...
class Project:
    ...
    def set_device(self):
        ...
        if self.accelerator == 'cuda' and torch.cuda.is_available():
            ...
            #  self.devices が整数かつ範囲内を確保
            idx_gpu = int(self.devices) if isinstance(self.devices, int) else 0
            if idx_gpu >= num_gpus:
                print(
                    f"Warning: Requested GPU index {idx_gpu} is out of range (only {num_gpus} GPUs available). Using GPU 0.")
                idx_gpu = 0

            name_gpu = torch.cuda.get_device_name(idx_gpu)
            device = torch.device(f"cuda:{idx_gpu}")
            torch.cuda.set_device(device)

            print(f"::: Available GPUs: {num_gpus}")
            print(f"::: Using GPU {idx_gpu}: {name_gpu}")
            print("--------------------------------------------------------------------")
            ...
            raise ValueError(f"The selected device '{self.accelerator}' is not supported.")
